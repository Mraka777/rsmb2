...
function mysql_addshot($bid, $lap, $shot, $res) { //добавляет в таблицу rsm_logs лог стрельбы 
  global $bir; 
  $sql_shot_insert = "INSERT INTO `cp827871_rsmb`.`rsm_logs` (`logs_id`, `race_id`, `b_id`, `lap`, `shot`, `res`) VALUES (NULL, '1', '$bid', '$lap', '$shot', '$res')";
...
function mysql_addmark($bid, $mark, $time) { //добавляет отметку в базу
  global $bir;
  $sql_mark_insert = "INSERT INTO `cp827871_rsmb`.`rsm_logr` (`logr_id`, `race_id`, `b_id`, `mark`, `time`) VALUES (NULL, '1', '$bid', '$mark', '$time');";
  //echo('WARNING: MYSQL ACTIVE <br>');
...
function run($bid,$id) { // Функция получает id и секунду, на выходе получаем пройденную дистанцию за секунду
  ...
  $passed=round($total_speed*1000/(60*60),3);
...
$jj=0;
$sql = "SELECT `length`, `x`, `z` FROM `rsm_track_point` WHERE `lap` = 1 ORDER BY `point_order` ASC LIMIT 0, 30 ";
...
echo("<img src=\"old/gd-test.php\"><br>");
...
$i=1;
foreach($race_sportsman_list as $sportsman_list) { //$sportsman_list['sportsman_id']
      $bir[$i]['biatloner_id']=$sportsman_list['sportsman_id'];
      $bir[$i]['biatloner_team_id']=$sportsman_list['team_id'];
      $bir[$i]['name1']=$sportsman_list['name1'];
      $bir[$i]['name2']=$sportsman_list['name2'];
      $bir[$i]['phys_energy']=$sportsman_list['phys_energy'];
      $bir[$i]['phys_strength']=$sportsman_list['phys_strength'];
      $bir[$i]['phys_endurance']=$sportsman_list['phys_endur'];
      $bir[$i]['shoot_technique']=$sportsman_list['shoot_tech'];
      $bir[$i]['shoot_calm']=$sportsman_list['shoot_calm'];
      $bir[$i]['shoot_accuracy']=$sportsman_list['shoot_acc'];
      $bir[$i]['track_technique']=$sportsman_list['track_tech'];
      $bir[$i]['track_speed']=$sportsman_list['track_spd'];
    //добавляем состояние
      $bir[$i]['status']='start';
      $bir[$i]['shooting_status']='prepare';//состояние на стрельбище prepare - пришел на стрельбище, ready - прошла изготовка
      $bir[$i]['hold']='0';
      $bir[$i]['lap']='1';
      $bir[$i]['fatique']='0';
      //$bir[$i]['race_log']='';
      //$bir[$i]['race_log']['1']['1']=0;
      //$bir[$i]['race_log']['1']['0']=0;
      $bir[$i]['distance']=0;
      $bir[$i]['mark']='1';
      $bir[$i]['segment_position']='1';
      
      $bir[$i]['dist_log']=array();
      ...
      if($b['hold']>1){$bir[$bid]['hold']--;} 
        ...
        if($b['status']=="start"){
          ...
          if($bir[$bid]['race_log'][$current_lap][$i]>$next_mark[$current_lap]) {   //проверка на отметку
            ...
            $mark_time=round($i-$time_delta_mark,2);
