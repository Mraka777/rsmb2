...
class Next_model extends CI_Model {
    ...
    function do_academy() {
      ...
      foreach ($teams as $team) {
        ...
        $sign_fee = $s_a['salary'] * 12;
        $sql = "INSERT INTO `rsm_sportsman_academy` (`sportsman_id`, `team_id`, `country_id`, `age`, `bday`, `name1`, `name2`, `team_num`, `phys_energy`, `phys_strength`, `phys_endur`, `shoot_tech`, `shoot_calm`, `shoot_acc`, `track_tech`, `track_spd`, `sportsman_prof`, `popularity`, `overall_rating`, `salary`, `days_actual`, `phys_strength_mvu`, `phys_endur_mvu`, `shoot_tech_mvu`, `shoot_calm_mvu`, `shoot_acc_mvu`, `track_tech_mvu`, `track_speed_mvu`, `academy_sign_fee`) VALUES (NULL, '".$team['team_id']."', '".$team['country_id']."', '".$s_a['age']."', '".$s_a['bday']."', '".$s_a['name1']."', '".$s_a['name2']."', '', '".$s_a['phys_energy']."', '".$s_a['phys_strength']."', '".$s_a['phys_endur']."', '".$s_a['shoot_tech']."', '".$s_a['shoot_calm']."', '".$s_a['shoot_acc']."', '".$s_a['track_tech']."', '".$s_a['track_spd']."', '".$s_a['sportsman_prof']."', '".$s_a['popularity']."', '".$s_a['overall_rating']."', '".$s_a['salary']."', '".$s_a['days_actual']."', '".$s_a['phys_strength_mvu']."', '".$s_a['phys_endur_mvu']."', '".$s_a['shoot_tech_mvu']."', '".$s_a['shoot_calm_mvu']."', '".$s_a['shoot_acc_mvu']."', '".$s_a['track_tech_mvu']."', '".$s_a['track_speed_mvu']."', '".$sign_fee."');";
        $this->db->query($sql);
    ...
    function change_balance($team_id, $op_type, $delta) {
      //Описание транзакций в `rsm_team_balance_operation`
...
WHERE team_id = ".$team_id." ";
    ...
    function delete_season() {
      ...
      $sql = "TRUNCATE TABLE `rsm_league_playoff`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_race_sportsman_tactics`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_team_statistic`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_board_user_favorite_topic`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_engine_log`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_team_basic_rating_log`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_sportsman_exp`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_sportsman_exp_log`";
      $this->db->query($sql);
      $sql = "TRUNCATE TABLE `rsm_sportsman_transfer_value`";
      $this->db->query($sql); 
    ...
    function generate_season($countries)
    {
      ...
      for ($cc=1; $cc<=$countries;$cc++) {
        ...
        for ($i = 1; $i <= $teams_country; $i++) {
          ...
          $this->db->query($sql);
          //expa
          $sql = "INSERT INTO `rsm_sportsman_exp` (`rsm_sportsman_exp_id`, `rsm_sportsman_sportsman_id`, `rsm_sportsman_exp`) VALUES (NULL, '".$s_id."', '0');";
          $this->db->query($sql);
          //transfer value
          $val_vbr = rand(1,10);
          $s_id_value = round(($rating * (100 + $val_vbr)),0);
          $sql = "INSERT INTO `rsm_sportsman_transfer_value` (`rsm_sportsman_transfer_value_id`, `sportsman_id`, `sportsman_transfer_value`) VALUES (NULL, '".$s_id."', '".$s_id_value."');";
          $this->db->query($sql);
          ...
          //$talent2 = rand($t_first,$t_second);$talent3 = rand($t_first,$t_second);$talent4 = rand($t_first,$t_second);$talent5 = rand($t_first,$t_second);$talent6 = rand($t_first,$t_second);$talent7 = rand($t_first,$t_second);
          
          
          //$talent1 = rand($t_first,$t_second);
          $rnd_p1 = -30;
          $rnd_p2 = 30;
          
          $rnd1 = rand($rnd_p1,$rnd_p2);
          $talent1 = $talent1 + $rnd1;
          if ($talent1 > 99) {
            $talent1=99;
          }
          elseif ($talent1 < 0) {
          ...
          $rnd3 = rand($rnd_p1,$rnd_p2);
          $talent3 = $talent3 + $rnd1;
          if ($talent3 > 99) {
            $talent3=99;
          }
          elseif ($talent3 < 0) {
          ...
          $rnd4 = rand($rnd_p1,$rnd_p2);
          $talent4 = $talent4 + $rnd1;
          if ($talent4 > 99) {
            $talent4=99;
          }
          elseif ($talent4 < 0) {
          ...
          $rnd5 = rand($rnd_p1,$rnd_p2);
          $talent5 = $talent5 + $rnd1;
          if ($talent5 > 99) {
            $talent5=99;
          }
          elseif ($talent5 < 0) {
          ...
          $rnd6 = rand($rnd_p1,$rnd_p2);
          $talent6 = $talent6 + $rnd6;
          if ($talent6 > 99) {
            $talent6=99;
          }
          elseif ($talent6 < 0) {
          ...
          $rnd7 = rand($rnd_p1,$rnd_p2);
          $talent7 = $talent7 + $rnd7;
          if ($talent7 > 99) {
            $talent7=99;
          }
          elseif ($talent7 < 0) {
        ...
        $i=1;$j=1;$infr_id = array ( 1=>1, 4, 7, 10, 13, 16, 19, 22);
        for ($i = 1; $i <= $teams_country; $i++) {
          for ($j = 1; $j <= 8; $j++) {
          //$sql = "INSERT INTO `rsm_infrastructure` (`infr_id`, `team_id`, `building_id`, `building_level_id`, `days_next`, `days_next`) VALUES (NULL, '".($i+($cc-1)*$teams_country)."', '".$j."', '".$infr_id[$j]."', '0');";
          ...
          for ($i = 1; $i <= $num_race; $i++) {
            ...
            foreach ($query->result_array() as $row)
            {
              for ($j = 1; $j <= $num_race; $j++) {
                ...
                //echo($sql.'<br>');
                $query = $this->db->query($sql);
                $sql = "INSERT INTO `rsm_race_sportsman_tactics` (`rsm_race_sportsman_tactics_id`, `race_id`, `sportsman_id`, `rsm_race_sportsman_tactics_importance`, `rsm_race_sportsman_tactics_ski_plain`, `rsm_race_sportsman_tactics_ski_hill`, `rsm_race_sportsman_tactics_shooting`) VALUES (NULL, '".$race_sp_list_id."', '".$row['sportsman_id']."', '1', '4', '7', '10');";
          
              //if ( $temp_race['race_id'] == '1'){
                //print($sql);
                //}
              $this->db->query($sql);
   ...
   function rsm_menu($segment, $param, $active, $user_id) {
...
WHERE rsm_league_promotion_rule.league_level_to = ".$query3['league_lvl']." AND rsm_league_promotion_rule.league_num_to = ".$query3['league_num']." AND rsm_league.country_id = ".$league['country_id']." ";
  ...
  function get_general_data($team_id, $menu1, $menu2, $language) {
		if ($language == 'en') {
      $language_id = 0;
    }
    elseif ($language == 'ru') {
    ...
    //country
    $sql = "SELECT rsm_country.logo, rsm_team.team_name, rsm_country.country_id FROM `rsm_country`
...
WHERE rsm_team.team_id =  ".$team_id." ";
    ...
    $sql = "SELECT rsm_menu_id, level1, num, glyph, level1_text FROM `rsm_menu` WHERE visible = 1 AND language_id = ".$language_id." ";
    $query = $this->db->query($sql);
 ...
 function get_page_help($user_id, $menu1, $menu2) {
  $sql = "SELECT country_id FROM `rsm_team` WHERE `user_id` = ".$user_id." ";
  
  $query=$this->db->query($sql);
  $query = $query->result_array();
  $country_id = $query[0]['country_id'];
  //print ($query);
  //print($user_id." ".$menu1." ".$menu2);
  $sql = "SELECT help_topic, help_text FROM `rsm_page_help` WHERE `path1` LIKE '".$menu1."' AND `path2` LIKE '".$menu2."' AND `country_id` = ".$country_id." ";
  //print($sql);
  $query=$this->db->query($sql);
  if ($query->num_rows() > 0) {
    $query = $query->result_array();
